...
final class MatchModel
{
    ...
    public static function insertMatch(array $m): int
    {
        return DB::insert(
            'INSERT INTO matches
                (ext_key, stage, round_name, group_name, team1, team2, kickoff, venue, score1, score2, status, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [
                $m['ext_key'] ?? null, $m['stage'] ?? 'group', $m['round_name'] ?? null,
    ...
    public static function updateFromImport(int $id, array $m): void
    {
        DB::run(
            'UPDATE matches SET
                stage = ?, round_name = ?, group_name = ?, team1 = ?, team2 = ?,
                kickoff = ?, venue = ?, score1 = ?, score2 = ?, status = ?, updated_at = ?
             WHERE id = ?',
            [
                $m['stage'] ?? 'group', $m['round_name'] ?? null, $m['group_name'] ?? null,
